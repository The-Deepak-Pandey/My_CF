...
void solve() {
    ...
    for(auto it : a){
        if(s > t) swap(s, t); // for invariant s <= t

        if(it <= s){
            // can fit in both without penalty
            // tighter vaale me daalo (s)
            s = it;
        } else if(it <= t){
            // can fit in t without penalty
            t = it;
        } else{
            // cannot fit in either without penalty
