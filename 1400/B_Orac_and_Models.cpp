...
int help(int ind, vector<int> &nums){
    ...
    // try jumping to multiples of ind sucht that nums[jump]%nums[ind] == 0

    int ans = 0;

    for(int jump = 2; jump * ind < nums.size(); jump++) {
        if(nums[jump * ind] > nums[ind]) {
            ans = max(ans, 1 + help(jump * ind, nums));
        }
    }
